...
function populateClassroomDataForDay(document, dayOffset, allClassroomData) {
    ...
    timeSlotLabels.forEach(label => {
        ...
        const slotData = allClassroomData.filter(item => item && item["空闲时段"] === suffix);
    ...
    timeSlotLabels.forEach(slotLabel => {
        ...
        if (!suffixMatch) return;
        const timeSlotSuffix = suffixMatch[1].replace(/[上午下午晚上昼间]/g, '').trim();

        Object.entries(allBuildingCodes).forEach(([buildingName, config]) => {
            const currentSlotDataBuilding = allClassroomData.filter(item => item && item["教学楼"] === buildingName && item["空闲时段"] === timeSlotSuffix);
            ...
            const formatAndStyleRooms = (roomList) => {
                ...
                return roomList.map(item => {
                    ...
                    const isUnderlined = timeSlotSuffix !== "1-2" && timeSlotSuffix !== "1-8" && !(previousClassrooms[buildingName]?.has(roomName));
                    const isStrikethrough = STRIKETHROUGH_APPLICABLE_SLOTS.includes(timeSlotSuffix) && !nextSlotClassrooms.has(roomName);
            ...
            if (config.floors) { // 工学馆逻辑
                ...
                if (roomCell) {
                    ...
                    if (buildingName === "科技楼") {
                        ...
                        const sortedZizhuRooms = zizhuRooms.sort((a, b) => (a.letter || "Z").localeCompare(b.letter || "Z"));
        ...
        if (timeSlotSuffix !== "1-8") {
            buildings.forEach(b => {
                previousClassrooms[b] = getAllClassroomsFromData(allClassroomData.filter(item => item && item["教学楼"] === b && item["空闲时段"] === timeSlotSuffix));
            });
        }
    });
}
...
async function generateFinalHtmlReport() {
    ...
    } catch (e) { console.error(`✖ 读取或解析事件文件失败: ${e.message}`); }
    try {
    ...
    } catch (e) { console.error(`✖ 读取或解析格言文件失败: ${e.message}`); }
        ...
        if (todayDateObj) {
            ...
            if (todayActiveEvents.length > 0) {
                ...
                todayActiveEvents.forEach(event => {
                    let rooms = event["是否占用全体教室"] ? "全体教室。" : `<strong>${event["占用教室"] || '未知'}</strong>教室。`;
                    emergencyHtml += `<p>📢 <strong>${event["名称"] || '无标题事件'}</strong>将于${event["起始日期"]} ${event["起始时间"]} - ${event["结束日期"]} ${event["结束时间"]}占用<strong>${event["占用教学楼"] || '未知楼宇'}</strong>${rooms}</p>`;
                });
    ...
    for (let dayOffset = 0; dayOffset < totalDays; dayOffset++) {
        ...
        try {
            ...
            console.error(`  ✖ 读取或解析 Day ${dayOffset} 的教室数据时发生错误: ${error.message}`);
        }
    ...
    } catch (e) { console.error(`  ✖ 比较线上版本时出错: ${e.message}`); }
